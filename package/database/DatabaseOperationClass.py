...
class DatabaseOperation:
    ...
    def database_connection(self, database_name):
        try:
            ...
            self._create_folder_if_not_exists(self.database_folder)
    ...
    def create_table_in_database(self):
        try:
            ...
            if cursor.fetchone()[0] == 1:
                ...
                self._update_or_create_columns_in_table_with_connection(column_names, database_connection)
    ...
    def _update_or_create_columns_in_table_with_connection(self, column_names, database_connection):
    ...
    def insert_data_in_database(self):
        ...
        for file in files_directory:
            self._insert_data_from_file_with_connection(file, connection)

    def _insert_data_from_file_with_connection(self, file, connection):
        try:
            with open(self.transformed_folder + '/' + file, "r") as f:
                next(f)
                reader = csv.reader(f, delimiter="\n")
                for column in enumerate(reader):
                    self._insert_column_into_csv_with_connection(column, connection)
    ...
    def _insert_column_into_csv_with_connection(self, column, connection):
